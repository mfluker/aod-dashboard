...
#     """
#     Fetches and parses job status data for a given status and date range.
#     Returns a cleaned DataFrame with ID, Order Type, Franchisee, Date, and Status.
#     """
#     if session is None:
...
#     # Strip HTML and parse CSV from raw text
#     cleaned = re.sub(r"<[^>]+>", "", resp.text).strip()
#     if not cleaned:
#         return pd.DataFrame()
...
#     # Find and assign the correct status date column
#     look = f"{status_name} Date".lower()
#     match = next((c for c in df.columns if c.lower() == look), None)
#     df["Status"] = status_name
#     df["Date"] = df[match] if match else pd.NaT
...
def download_conversion_report(start_date: str,
    ...
    df["Outbound Proxy Value"] = df["Outbound Communication Count"].astype(int)
    df["Inbound Help Rate (%)"]  = df["Inbound Rate Value"].map("{:.1f}%".format)
    df["Outbound Help Rate (%)"] = df["Outbound Help Rate"].str.rstrip("%").astype(float).map("{:.1f}%".format)
